...
export default function FollowupSearch({days}:{days:number}){
 ...
 return <section className="search-followup">
  ...
  {loading&&<div className="followup-progress"><LoaderCircle className="spin"/><strong>{progress[step]}</strong><p>검색 범위에 따라 20초 이상 걸릴 수 있습니다. 화면을 닫지 말고 잠시 기다려 주세요.</p><div><i className={step>=0?'active':''}/><i className={step>=1?'active':''}/><i className={step>=2?'active':''}/></div></div>}
